...
export function TemplateKeuangan() {
  const { totalSaldo, pemasukan, pengeluaran, targetDonasi, terkumpulDonasi } = KEUANGAN_BARU;
  ...
  return (
    ...
    >
      ...
      <motion.div className="text-center mb-6" variants={itemVariants}>
        <p className="text-sm uppercase tracking-widest text-brand-secondary/60 font-bold mb-1">Total Saldo</p>
      ...
      <motion.div className="flex flex-row gap-8 mb-6" variants={itemVariants}>
        ...
        <section className="flex-1 bg-white/70 backdrop-blur-md p-6 rounded-2xl border border-brand-primary/10 shadow-sm">
          <div className="flex items-center gap-3 mb-4 pb-3 border-b-2 border-brand-primary/20">
            ...
            <h3 className="text-2xl font-bold text-brand-secondary">Pemasukan</h3>
          </div>
          <div className="flex flex-col">
            {pemasukan.map((item, idx) => <FinanceItem key={idx} {...item} formatFn={formatRp} />)}
          </div>
        </section>
        ...
        <section className="flex-1 bg-white/70 backdrop-blur-md p-6 rounded-2xl border border-brand-primary/10 shadow-sm">
          <div className="flex items-center gap-3 mb-4 pb-3 border-b-2 border-brand-primary/20">
            ...
            <h3 className="text-2xl font-bold text-brand-secondary">Pengeluaran</h3>
          </div>
          <div className="flex flex-col">
            {pengeluaran.map((item, idx) => <FinanceItem key={idx} {...item} formatFn={formatRp} />)}
          </div>
        </section>
      </motion.div>

      {/* Progress Bar Section (Donasi) */}
      <motion.section className="bg-white/90 backdrop-blur-md p-6 rounded-2xl border border-brand-primary/20 shadow-md relative overflow-hidden" variants={itemVariants}>
        {/* Dekorasi tipis */}
        <div className="absolute right-0 top-0 w-24 h-24 bg-brand-primary/10 rounded-full blur-2xl -mr-8 -mt-8"></div>
        
        <div className="flex justify-between items-end mb-3 relative z-10">
          <div>
            <h3 className="text-xl font-bold text-brand-secondary mb-1">Progress Donasi Pembangunan</h3>
            <p className="text-brand-secondary/70 text-sm font-medium">Target: {formatRp(targetDonasi)}</p>
          </div>
          <div className="text-right">
            <p className="text-3xl font-bold text-brand-primary mb-1">{persen.toFixed(0)}%</p>
            <p className="text-xs font-semibold text-brand-secondary/60 uppercase">Kurang {formatRp(kekurangan)}</p>
          </div>
        </div>

        {/* Bar Visual */}
        <div className="w-full h-5 bg-brand-secondary/10 rounded-full overflow-hidden relative z-10 shadow-inner">
          <motion.div 
          ...
          >
            {/* Animasi cahaya di dalam bar */}
            <div className="absolute inset-0 bg-white/20 w-1/2 skew-x-12 animate-[marquee_2s_infinite]"></div>
          </motion.div>
        </div>
