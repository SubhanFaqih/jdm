...
export function TemplateKeuangan() {
  const { totalSaldo, pemasukan, pengeluaran, targetDonasi, terkumpulDonasi, namaProgram, pemasukanBulanIni, pengeluaranBulanIni, hasActiveDonasi, isLoading } =
    useFinanceReport();
  ...
  // Kalkulasi Progress
  const persen = targetDonasi > 0 ? Math.min((terkumpulDonasi / targetDonasi) * 100, 100) : 0;
      ...
      <motion.div className="flex flex-row gap-8 mb-6" variants={itemVariants}>
        ...
        <section className="flex-1 bg-white/70 backdrop-blur-md p-6 rounded-2xl border border-brand-primary/10 shadow-sm">
          <div className="flex items-center gap-3 mb-4 pb-3 border-b-2 border-brand-primary/20">
            ...
            <h3 className="text-2xl font-bold text-brand-secondary flex justify-between items-center w-full">
              <span>Pemasukan {namaBulan}</span>
              <span className="text-emerald-600 font-extrabold">{formatRp(pemasukanBulanIni)}</span>
            </h3>
          </div>
        ...
        <section className="flex-1 bg-white/70 backdrop-blur-md p-6 rounded-2xl border border-brand-primary/10 shadow-sm">
          <div className="flex items-center gap-3 mb-4 pb-3 border-b-2 border-brand-primary/20">
            ...
            <h3 className="text-2xl font-bold text-brand-secondary flex justify-between items-center w-full">
              <span>Pengeluaran {namaBulan}</span>
              <span className="text-rose-600 font-extrabold">{formatRp(pengeluaranBulanIni)}</span>
            </h3>
          </div>
          <div className="flex flex-col">
